...
class ObjectsEngine extends AbstractEngine
{
    ...
    protected $commonObjectTable;
    protected $commonObjectIdField;
    
    protected $commonObjectProperties = array();
    ...
    protected function getFolderTreeHelper()
    {
        ...
        return $this->folderTreeHelper = new \Zend\Db\Sql\Helper\Tree\NestedSet(array(
            'adapter' => $this->adapter,
            'table'   => $folderType['localName'],
            'columns' => array(
    ...
    public function getFolderTree($folderId, $optional = array())
    {
        ...
        $folderTree = $this->adapter->query(
            $this->sqlBuilder->buildSqlString($query, $this->adapter),
            'execute',
            $resultSetPrototype
        );
        if (!$folderTree->count()) {
        ...
        $statement = preg_replace_callback(
            ...
            function ($cmisKey) use ($adapterPlatform, $useFullViews) {
                ...
                if ($useFullViews && ($type = $this->repositoryEngine->getTypeDefinition($key)) !== null) {
                    ...
                    $typeQuery = $this->sqlBuilder->buildSqlString($typeQuery, $this->adapter);
    ...
    public function _selectObject($filter = null)
    {
        ...
        if (!$filter) {
            return $select->columns($this->commonObjectProperties);
        }
        if (is_string($filter)) {
        ...
        foreach($filter as $alias => &$field) {
            $field = $this->commonObjectProperties[$alias];
        }
    ...
    public function _selectObjectIdByNameInFolder($folderId, $objectName)
    {
        ...
        $select
            ...
            ->where(array(
                ...
                'obj.'.$this->commonObjectProperties['cmis:name'] => $objectName,
            ));
        return $select;
    }
